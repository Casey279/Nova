...
# Sleep prevention was removed as per user request
# This comment is left to indicate where the SleepPrevention class was removed
...
class ChroniclingAmericaTab(QWidget):
    ...
    def create_middle_panel(self):
        ...
        # No pagination controls needed - all results are displayed at once
    ...
    def create_right_panel(self):
        ...
        self.checkpoint_button = QPushButton("Resume Downloads")
        self.checkpoint_button.clicked.connect(self.open_checkpoint_dialog)
        self.checkpoint_button.setToolTip("View and resume interrupted downloads")

        # Check if we have any checkpoints available
        self.checkpoint_button.setEnabled(self._has_checkpoints())
        ...
        download_button_layout.addWidget(self.checkpoint_button)
    ...
    def search_results_received(self, pages, pagination, extra_info=None):
        ...
        status_text = f"Found {total_items} results."
    ...
    # Pagination methods removed - all results are displayed at once
    ...
    def download(self, selected_only=False, checkpoint_id=None):
        """
        Download and optionally import newspaper content with checkpoint support.

        Args:
            selected_only: Whether to download only selected items
            checkpoint_id: Optional checkpoint ID to resume from
        """
        # If we're resuming from a checkpoint, use the checkpoint's settings
        if checkpoint_id:
            # Import required modules
            from .download_checkpoint import DownloadCheckpoint
            from .resumable_search_worker import ResumableSearchWorker

            # Get checkpoint manager
            checkpoint_manager = DownloadCheckpoint(self.download_directory)
            ...
            if not checkpoint_data:
                QMessageBox.warning(
                    self,
                    "Checkpoint Error",
                    f"Could not load checkpoint data. The checkpoint may be corrupted or deleted."
                )
                return

            # Extract parameters from checkpoint
            search_params = checkpoint_data.get('search_params', {})
            download_formats = checkpoint_data.get('download_formats', [])
            max_pages = checkpoint_data.get('max_pages', 100)

            # Show what we're resuming
            completed = checkpoint_data.get('completed_count', 0)
            total = checkpoint_data.get('total_pages', 0)
            percent = round(completed / max(total, 1) * 100, 1)

            # Check if user wants to resume
            message = f"Resume download from checkpoint?\n\n"
            message += f"Progress: {completed} of {total} pages ({percent}%)\n"
            message += f"Formats: {', '.join(download_formats)}\n\n"

            # Add info about what is being downloaded
            ...
            if date_start and date_end:
                message += f"Date Range: {date_start} to {date_end}\n"
            ...
            # Create worker for resumable download/import
            self.download_worker = ResumableSearchWorker(
            ...
            self.download_worker.checkpoint_signal.connect(self.update_checkpoint_status)

            # Update UI
            self.status_label.setText(f"Resuming download: {completed} of {total} pages completed...")
        ...
        # Not resuming, proceed with normal download
        ...
        if selected_only:
            ...
            if date_start and date_end:
                ...
                # and suggest using checkpointing
                if is_large_download:
                    download_limit = 20 * max_pages  # ~2000 newspaper pages

                    message = (
                        f"Your date range ({date_start} to {date_end}) may contain approximately "
                        f"{estimated_pages} newspaper pages, which exceeds the recommended "
                        f"download limit of {download_limit} pages per operation.\n\n"
                        f"The system will automatically create checkpoints during the download, "
                        f"allowing you to resume if the download is interrupted.\n\n"
                        f"Would you like to continue with this large download?"
                    )
        ...
        # Create worker for download/import with checkpointing
        try:
            from .resumable_search_worker import ResumableSearchWorker
            ...
            self.download_worker = ResumableSearchWorker(
                ...
                import_service=self.import_service if import_to_db else None,
                enable_checkpointing=True,
                checkpoint_interval=20  # Create checkpoint every 20 pages
            )
            ...
            # Fall back to regular SearchWorker if ResumableSearchWorker is not available
    ...
    def download_completed(self, results):
        ...
        checkpoint_id = results.get('checkpoint_id')  # Get checkpoint ID if download was interrupted

        # Update UI
        self.progress_bar.setValue(100)

        # If we have a checkpoint ID, the download was interrupted
        if checkpoint_id:
            self.status_label.setText(
                f"Download interrupted and saved to checkpoint. Use 'Resume Downloads' to continue."
            )

            # Show result message for interrupted download
            message = f"Download interrupted.\n\n"
            message += f"Downloaded so far: {total_downloaded} items.\n"
            if self.import_check.isChecked():
                message += f"Imported so far: {total_imported} items.\n"
            message += f"Skipped (already in database): {total_skipped} items.\n\n"
            message += f"A checkpoint has been saved. You can resume this download later by clicking 'Resume Downloads'."

            QMessageBox.information(self, "Download Interrupted", message)
        else:
            # Normal completion
            if self.import_check.isChecked():
                self.status_label.setText(
            ...
            if self.import_check.isChecked():
                # Get unique publication names/LCCNs that were processed
                unique_publications = set()
                for item in successful:
                    lccn = item.get('lccn', '')
                    title = item.get('title', 'Unknown')
                    if lccn and title:
                        # Use just the base part of the title (without volume/date info)
                        base_title = title
                        if "[volume]" in base_title:
                            base_title = base_title.split("[volume]")[0].strip()
                        unique_publications.add((lccn, base_title))

                # Add database import information
                message += f"Downloaded: {total_downloaded} items.\n"
                message += f"Database Import Results:\n"
                message += f"• {len(unique_publications)} publication(s) recorded in Sources table.\n"
                message += f"• {total_imported} newspaper pages recorded in NewspaperPages table.\n"
                message += f"• {total_skipped} pages skipped (already in database).\n"
                message += f"• {len(failed)} failed imports.\n\n"

                # List the publications that were imported
                if unique_publications:
                    message += "Publications imported:\n"
                    for lccn, title in unique_publications:
                        message += f"• {title} (LCCN: {lccn})\n"
                    message += "\n"
        ...
        # Make sure the checkpoint button shows the right state
        if hasattr(self, 'checkpoint_button'):
            self.checkpoint_button.setEnabled(self._has_checkpoints())
    ...
    def set_ui_enabled(self, enabled):
        ...
        # Make sure checkpoint button is always available if we have checkpoints
        if hasattr(self, 'checkpoint_button'):
            # Enable only if we're not in the middle of an operation or there are checkpoints
            self.checkpoint_button.setEnabled(enabled or self._has_checkpoints())
    ...
    def update_checkpoint_status(self, checkpoint_id, completed, total):
        """
        Update the UI with checkpoint status.
        ...
        # Update status label
        percent = round(completed / max(total, 1) * 100, 1)
        self.status_label.setText(f"Created checkpoint: {completed} of {total} pages ({percent}%)")

        # Add a note that this can be resumed if interrupted
        print(f"Checkpoint created: {checkpoint_id} - {completed}/{total} pages ({percent}%)")

    def open_checkpoint_dialog(self):
        """Open the dialog for managing download checkpoints."""
        try:
            # Import the checkpoint dialog
            from .checkpoint_dialog import CheckpointDialog

            # Create dialog
            dialog = CheckpointDialog(self.download_directory, self)

            # Connect the resume signal
            dialog.resume_checkpoint.connect(self.resume_from_checkpoint)
            ...
            QMessageBox.warning(
                self,
                "Checkpoint Error",
                f"Error opening checkpoint dialog: {str(e)}"
            )

    def resume_from_checkpoint(self, checkpoint_id):
        """
        Resume download from a checkpoint.

        Args:
            checkpoint_id: Checkpoint ID to resume from
        """
        # Start a download with the checkpoint ID
        self.download(selected_only=False, checkpoint_id=checkpoint_id)
